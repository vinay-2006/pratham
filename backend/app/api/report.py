"""
GET /api/report/{intake_id} — Consolidated Clinical Intelligence Report

Single endpoint that returns ALL sections needed for the doctor's report page:
  1. Patient Summary
  2. Vitals
  3. Symptoms
  4. NLP Findings
  5. Risk Engine Scores
  6. Lab Intelligence (XGBoost)
  7. Imaging Intelligence (EfficientNetB0)
  8. Aggregation Engine results + evidence breakdown
  9. Evidence list

Avoids 8+ separate API calls from the frontend.
"""
...
async def get_clinical_report(intake_id: str):
    """
    Consolidated Clinical Intelligence Report.
    ...
    try:
        ...
        nlp_data, lab_data, imaging_data, agg_data, evidence_list = await asyncio.gather(
